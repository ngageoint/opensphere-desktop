...
/*			configFileProvider(
	            ...
	            sh "${env.mvnHome}/bin/mvn --no-snapshot-updates -Dmaven.repo.local=${env.LOCAL_REPO} -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn clean install deploy -Pautomated ${env.REMOTE_REPO_ARGS}"
	        /*}*/
        } catch (error) {
            notifyFailed();
            throw error;
