    ...
    stage ('Compile') {
        try {
			...
			configFileProvider(
	            ...
	            sh "${env.mvnHome}/bin/mvn -Djavax.net.debug=ssl -X -s $MAVEN_SETTINGS --no-snapshot-updates -Dmaven.repo.local=${env.LOCAL_REPO} clean install deploy -Pautomated ${env.REMOTE_REPO_ARGS}"
	        }
        } catch (error) {
            notifyFailed();
            throw error;
