...
/**
 * Unit test for {@link GimbalOrientationParser}.
 */
public class GimbalOrientationParserTest
{
    /**
    ...
    public void test() throws IOException, ParseException
    {
        ...
        GimbalOrientationParser parser = new GimbalOrientationParser(uiRegistry);
        ...
        assertEquals(70.96, metadatas.get(0).getCameraYawAngle(), 0d);
        assertEquals(-58.82, metadatas.get(0).getCameraPitchAngle(), 0d);
        assertEquals(0.1, metadatas.get(0).getCameraRollAngle(), 0d);

        assertEquals(DateTimeUtilities.parseISO8601Date("2015-12-19T21:01:29.321Z").getTime(),
                metadatas.get(1).getTime().getTime());
        assertEquals(71.06, metadatas.get(1).getCameraYawAngle(), 0d);
        assertEquals(-59.8, metadatas.get(1).getCameraPitchAngle(), 0d);
        assertEquals(1.2, metadatas.get(1).getCameraRollAngle(), 0d);

        assertEquals(DateTimeUtilities.parseISO8601Date("2015-12-19T21:01:29.521Z").getTime(),
                metadatas.get(2).getTime().getTime());
        assertEquals(70.83, metadatas.get(2).getCameraYawAngle(), 0d);
        assertEquals(-56.27, metadatas.get(2).getCameraPitchAngle(), 0d);
        assertEquals(2.2, metadatas.get(2).getCameraRollAngle(), 0d);
    ...
    {
        Output output = new Output("gimbalAtt");
