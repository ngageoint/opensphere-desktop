 ...
 */
public class WFS110FilterToDataFilterConverter implements Function<CustomBinaryLogicOpType, Filter>
{
     ...
     */
    public static WFS110FilterToDataFilterConverter noInList()
    {
        WFS110FilterToDataFilterConverter w = new WFS110FilterToDataFilterConverter();
        w.ignoreInList = true;
        return w;
    }

    @Override
    public Filter apply(CustomBinaryLogicOpType ogcFilter)
    {
        Filter filter = new Filter(ogcFilter.getTitle(), new Source());
        filter.getOtherSources().add(filter.getSource());
        filter.setActive(ogcFilter.isActive(), this);
        filter.setMatch(ogcFilter.getMatch());
        if (ogcFilter.getFilterDescription() != null)
        {
            filter.setFilterDescription(ogcFilter.getFilterDescription());
        }
        filter.setFilterGroup(new Group());
        convert(ogcFilter.getComparisonOpsOrSpatialOpsOrLogicOps(), filter, null, null);
        return filter;
    }
