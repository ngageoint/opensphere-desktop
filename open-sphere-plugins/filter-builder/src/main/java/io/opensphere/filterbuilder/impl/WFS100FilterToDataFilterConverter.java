...
import net.opengis.ogc._100t.BinaryComparisonOpType;
import net.opengis.ogc._100t.BinaryLogicOpType;
import net.opengis.ogc._100t.FilterType;
import net.opengis.ogc._100t.LiteralType;
import net.opengis.ogc._100t.PropertyIsLikeType;
import net.opengis.ogc._100t.PropertyIsNullType;
import net.opengis.ogc._100t.PropertyNameType;
import net.opengis.ogc._100t.UnaryLogicOpType;

/**
 * Converts an OGC_110 filter to the application's internal 'Filter' structure.
 */
public class WFS100FilterToDataFilterConverter implements Function<FilterType, Filter>
{
    @Override
    public Filter apply(FilterType ogcFilter)
    {
        Source source = new Source();
        source.setActive(ogcFilter.isActive());
        Filter filter = new Filter(ogcFilter.getTitle(), source);
        filter.getOtherSources().add(source);
        filter.setActive(ogcFilter.isActive(), this);
        filter.setMatch(ogcFilter.getMatch());
        if (ogcFilter.getDescription() != null)
        {
            filter.setFilterDescription(ogcFilter.getDescription());
        }
    ...
    /**
     * Recursively iterates over an OGC filter and converts it into the application
     * type 'Filter'.
     *
     ...
     * @param criteria the criteria
     */
    private void convert(JAXBElement<?> element, Filter filter, Group group, Criteria criteria)
    {
        ...
        {
            doBinaryComparisonOpTypeAction(filter, group,
                    (BinaryComparisonOpType)val, element.getName().getLocalPart());
        }
    ...
    {
        return lit.getContent().stream().filter(v -> v != null).map(v -> v.toString())
                .findAny().orElse("");
    }
    ...
    {
        return (LiteralType)bcot.getExpression().stream().map(op -> op.getValue())
                .filter(v -> v instanceof LiteralType).findAny().orElse(null);
    }
    ...
    {
        return (PropertyNameType)bcot.getExpression().stream().map(op -> op.getValue())
                .filter(v -> v instanceof PropertyNameType).findAny().orElse(null);
    }
}
