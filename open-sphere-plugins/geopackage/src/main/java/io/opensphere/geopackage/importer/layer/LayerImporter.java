...
public class LayerImporter {
	 ...
	 *
	 * @return The created progress reporter used to notify progress of import to
	 *         user.
	 */
	public ProgressReporter importLayers(GeoPackage geopackage, List<GeoPackageLayer> layers,
		...
		for (String featureTable : featureTables) {
			...
			GeoPackageFeatureLayer featureLayer = new GeoPackageFeatureLayer(packageName, path, featureTable,
					featureDao.count());
		...
		DefaultCacheDeposit<GeoPackageLayer> deposit = new DefaultCacheDeposit<>(category, New.list(layerAccessor),
				layers, true, new Date(Long.MAX_VALUE), false);
	...
	/**
	 * Builds a {@link GeoPackageTileLayer} that represents the tile table specified
	 * by tile.
	 *
	 ...
	 * @param extensions  The list of extensions contained in the geopackage file.
	...
	private GeoPackageTileLayer buildTileLayer(TileDao tileDao, String packageName, String path, String tile,
		...
		if (!StringUtils.equals(String.valueOf(ProjectionConstants.EPSG_WORLD_GEODETIC_SYSTEM),
				tileProjection.getCode())) {
			ProjectionTransform layerToGeodetic = tileProjection
					.getTransformation(ProjectionConstants.EPSG_WORLD_GEODETIC_SYSTEM);
			GeometryEnvelope envelope = layerToGeodetic.transform(new GeometryEnvelope(box.getMinLongitude(),
