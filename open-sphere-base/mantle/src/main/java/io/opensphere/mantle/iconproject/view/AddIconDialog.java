...
public class AddIconDialog extends JFXDialog
{
    ...
    public AddIconDialog(Window owner, PanelModel thePanelModel)
    {
        ...
        setAcceptEar(() -> System.out.println("succesfully attained close exit shit"));

    }

    private void LoadfromFolder()
    {
        String colName = myPanelModel.getImportProps().getCollectionName().get();
        if (!StringUtils.isBlank(colName))
        {
            ImportProp pnl = myPanelModel.getImportProps();
            String subcategory = pnl.getSubCollectionName().get();
            if (subcategory != null)
            {
                ...
                if (result == JFileChooser.APPROVE_OPTION)
                {
                    ...
                    catch (IOException e)
                    {
                        JOptionPane.showMessageDialog(myOwner,
                                "Failed to load one or more icons: " + resultFile.getAbsolutePath(), "Image Load Error",
