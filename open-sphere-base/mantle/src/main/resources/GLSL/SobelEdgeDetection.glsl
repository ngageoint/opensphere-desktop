// TODO this is almost exactly like Prewitt edge detection, they could be combined.
vec4 getFragColor(sampler2D inTexture)
{
    ...
    vec4 horizEdge = value2 + (2.0*value5) + value8 - (value0 + (2.0*value3) + value6);
    vec4 vertEdge = value0 + (2.0*value1) + value2 - (value6 + (2.0*value7) + value8);
