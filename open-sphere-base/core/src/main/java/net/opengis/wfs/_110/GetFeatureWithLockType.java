...
/**
 * 
 * A GetFeatureWithLock request operates identically to a GetFeature request
 * expect that it attempts to lock the feature instances in the result set and
 * includes a lock identifier in its response to a client. A lock identifier is
 * an identifier generated by a Web Feature Service that a client application
 * can use, in subsequent operations, to reference the locked set of feature
 * instances.
 * 
 * 
 * <p>
