 ...
 * restricted if needed for specific OWS use, to include the specific Dataset
 * description metadata needed.
 * 
 * <p>
...
@XmlType(name = "DatasetDescriptionSummaryBaseType", propOrder = { "wgs84BoundingBox", "identifier", "boundingBox", "metadata",
...
public class DatasetDescriptionSummaryBaseType extends DescriptionType
{
     ...
     * coverage data, using the WGS 84 CRS with decimal degrees and longitude
     * before latitude. If no WGS 84 bounding box is recorded for a coverage,
     * any such bounding boxes recorded for a higher level in a hierarchy of
     * datasets shall apply to this coverage. If WGS 84 bounding box(es) are
     * recorded for a coverage, any such bounding boxes recorded for a higher
     * level in a hierarchy of datasets shall be ignored. For each lowest-level
     * coverage in a hierarchy, at least one applicable WGS84BoundingBox shall
     * be either recorded or inherited, to simplify searching for datasets that
     * might overlap a specified region. If multiple WGS 84 bounding boxes are
     * included, this shall be interpreted as the union of the areas of these
     * bounding boxes. Gets the value of the wgs84BoundingBox property.
     * 
     * <p>
    ...
    /**
     * Unordered list of zero or more minimum bounding rectangles surrounding
     * coverage data, in AvailableCRSs. Zero or more BoundingBoxes are allowed
     * in addition to one or more WGS84BoundingBoxes to allow more precise
     * specification of the Dataset area in AvailableCRSs. These Bounding Boxes
     * shall not use any CRS not listed as an AvailableCRS. However, an
     * AvailableCRS can be listed without a corresponding Bounding Box. If no
     * such bounding box is recorded for a coverage, any such bounding boxes
     * recorded for a higher level in a hierarchy of datasets shall apply to
     * this coverage. If such bounding box(es) are recorded for a coverage, any
     * such bounding boxes recorded for a higher level in a hierarchy of
     * datasets shall be ignored. If multiple bounding boxes are included with
     * the same CRS, this shall be interpreted as the union of the areas of
     * these bounding boxes. Gets the value of the boundingBox property.
     * 
     * <p>
     * This accessor method returns a reference to the live list, not a
     * snapshot. Therefore any modification you make to the returned list will
    ...
    /**
     * Optional unordered list of additional metadata about this dataset. A list
     * of optional metadata elements for this dataset description could be
    ...
    /**
     * Metadata describing zero or more unordered subsidiary datasets available
     * from this server. Gets the value of the datasetDescriptionSummary
     * property.
     * 
     * <p>
     * This accessor method returns a reference to the live list, not a
