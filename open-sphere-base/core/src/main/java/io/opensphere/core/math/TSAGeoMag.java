...
/* PUBLIC DOMAIN NOTICE This program was prepared by Los Alamos National
 * Security, LLC at Los Alamos National Laboratory (LANL) under contract No.
 * DE-AC52-06NA25396 with the U.S. Department of Energy (DOE). All rights in the
...
/* License Statement from the NOAA The WMM source code is in the public domain
 * and not licensed or under copyright. The information and software may be used
 * freely by the public. As required by 17 U.S.C. 403, third parties producing
 * copyrighted works consisting predominantly of the material produced by U.S.
 * government agencies must provide notice with such work(s) identifying the
 * U.S. Government material incorporated and stating that such material is not
 ...
 * </p>
 * <p>
 * <b>NOTE: </b>Comment out the logger references, and put back in the
 ...
 * <p>
 *
 * Verified by a JUnit test using the test values distributed with the 2015
 * update.
 * </p>
 * <p>
 *
...
{
    /**
     * A logger for this class. Every class MUST have this field, if you want to
     * log from this class. The class name is the fully qualified class name of
     * the class, such as java.lang.String. If you're not going to use log4j,
     * then comment all references to the logger, and uncomment the
     * System.***.println statements.
     */
     ...
     * input file. Added so that all data was internal, so that applications do
     * not have to mess with carrying around a data file. In the TSAGeoMag
     * Class, the columns in this file are as follows: n, m, gnm, hnm, dgnm,
     * dhnm
     */
    private final String[] input = { "    2015.0            WMM-2015        12/15/2014",
    ...
    /**
     * Geodetic altitude in km. An input, but set to zero in this class. Changed
     * back to an input in version 5. If not specified, then is 0.
     */
    ...
    /**
     * Geomagnetic grid variation, referenced to grid North. Not calculated or
     * output in version 5.0.
     */
    // private double gv = 0;
    ...
    /**
     * Added in version 5. In earlier versions the date for the calculation was
     * held as a constant. Now the default date is set to 2.5 years plus the
     * epoch read from the input file.
     */
    private double defaultDate = 2017.5;

    /**
     * Added in version 5. In earlier versions the altitude for the calculation
     * was held as a constant at 0. In version 5, if no altitude is specified in
     * the calculation, this altitude is used by default.
     */
    ...
    /**
     * The date in years, for the start of the valid time of the fit
     * coefficients
     */
    private double epoch;

    /**
     * bx is the north south field intensity by is the east west field intensity
     * bz is the vertical field intensity positive downward bh is the horizontal
     * field intensity
     */
    ...
    private double r, d, ca, sa, ct, st; // even though these only occur in one
                                         // method, they must be
                                         // created here, or won't have correct
                                         // values calculated
     ...
     * present, or an IO exception occurs, then the internal values valid for
     * 2015 will be used. Note that the last line of the WMM.COF file must be
     * 9999... for this method to read in the input file properly.
     */
    private void initModel()
    {
        ...
        catch (FileNotFoundException e)
        {
             ...
             * System.out.println("Error in TSAGeoMag.InitModel()"); System.out.
             * println("The input file WMM.COF was not found in the same");
             * System.out.println("directory as the application."); System.out.
             * println("The magnetic field components are set to internal values."
             * ); */
            setCoeff();
        }
        ...
        catch (IOException e)
        {
             ...
             * System.out.println("Error:  " + message);
             * System.out.println("Error in TSAGeoMag.InitModel()"); System.out.
             * println("The input file WMM.COF was found, but there was a problem "
             * ); System.out.println("reading the data."); System.out.
             * println("The magnetic field components are set to internal values."
             * ); */
            setCoeff();
        }
     ...
     * REFERENCED TO GRID NORTH OF POLAR STEREOGRAPHIC PROJECTION) OF THE
     * EARTH'S MAGNETIC FIELD IN GEODETIC COORDINATES FROM THE COEFFICIENTS OF
     * THE CURRENT OFFICIAL DEPARTMENT OF DEFENSE (DOD) SPHERICAL HARMONIC WORLD
     * MAGNETIC MODEL (WMM-2010). THE WMM SERIES OF MODELS IS UPDATED EVERY 5
     * YEARS ON JANUARY 1'ST OF THOSE YEARS WHICH ARE DIVISIBLE BY 5 (I.E. 1980,
     * 1985, 1990 ETC.) BY THE NAVAL OCEANOGRAPHIC OFFICE IN COOPERATION WITH
     * THE BRITISH GEOLOGICAL SURVEY (BGS). THE MODEL IS BASED ON GEOMAGNETIC
     ...
     *
     *
     * <b>ACCURACY:</b> IN OCEAN AREAS AT THE EARTH'S SURFACE OVER THE ENTIRE 5
     * YEAR LIFE OF A DEGREE AND ORDER 12 SPHERICAL HARMONIC MODEL SUCH AS
     * WMM-95, THE ESTIMATED RMS ERRORS FOR THE VARIOUS MAGENTIC COMPONENTS ARE:
     * </p>
     * <ul>
     * DEC - 0.5 Degrees<br>
     * DIP - 0.5 Degrees<br>
     ...
     * TRIGONOMETRIC RELATIONS WILL HAVE THE FOLLOWING APPROXIMATE ERRORS OVER
     * OCEAN AREAS:
     * </p>
     * <ul>
     * X - 140 nT (North)<br>
     ...
     *
     * <p>
     * OVER LAND THE RMS ERRORS ARE EXPECTED TO BE SOMEWHAT HIGHER, ALTHOUGH THE
     * RMS ERRORS FOR DEC, DIP AND GV ARE STILL ESTIMATED TO BE LESS THAN 0.5
     * DEGREE, FOR THE ENTIRE 5-YEAR LIFE OF THE MODEL AT THE EARTH's SURFACE.
     * THE OTHER COMPONENT ERRORS OVER LAND ARE MORE DIFFICULT TO ESTIMATE AND
     * SO ARE NOT GIVEN.
     * </p>
     * <p>
     *
     * THE ACCURACY AT ANY GIVEN TIME OF ALL FOUR GEOMAGNETIC PARAMETERS DEPENDS
     * ON THE GEOMAGNETIC LATITUDE. THE ERRORS ARE LEAST AT THE EQUATOR AND
     * GREATEST AT THE MAGNETIC POLES.
     * </p>
     * <p>
     *
     * IT IS VERY IMPORTANT TO NOTE THAT A DEGREE AND ORDER 12 MODEL, SUCH AS
     * WMM-2010 DESCRIBES ONLY THE LONG WAVELENGTH SPATIAL MAGNETIC FLUCTUATIONS
     * DUE TO EARTH'S CORE. NOT INCLUDED IN THE WMM SERIES MODELS ARE
     * INTERMEDIATE AND SHORT WAVELENGTH SPATIAL FLUCTUATIONS OF THE GEOMAGNETIC
     * FIELD WHICH ORIGINATE IN THE EARTH'S MANTLE AND CRUST. CONSEQUENTLY,
     * ISOLATED ANGULAR ERRORS AT VARIOUS POSITIONS ON THE SURFACE (PRIMARILY
     * OVER LAND, IN CONTINENTAL MARGINS AND OVER OCEANIC SEAMOUNTS, RIDGES AND
     * TRENCHES) OF SEVERAL DEGREES MAY BE EXPECTED. ALSO NOT INCLUDED IN THE
     * MODEL ARE NONSECULAR TEMPORAL FLUCTUATIONS OF THE GEOMAGNETIC FIELD OF
     * MAGNETOSPHERIC AND IONOSPHERIC ORIGIN. DURING MAGNETIC STORMS, TEMPORAL
     * FLUCTUATIONS CAN CAUSE SUBSTANTIAL DEVIATIONS OF THE GEOMAGNETIC FIELD
     * FROM MODEL VALUES. IN ARCTIC AND ANTARCTIC REGIONS, AS WELL AS IN
     * EQUATORIAL REGIONS, DEVIATIONS FROM MODEL VALUES ARE BOTH FREQUENT AND
     * PERSISTENT.
     * </p>
     * <p>
     *
     ...
     * (REGIONAL OR LOCAL) SURVEYS BE PERFORMED AND MODELS PREPARED BY THE USGS,
     * WHICH OPERATES THE US GEOMAGNETIC OBSERVATORIES. REQUESTS OF THIS NATURE
     * SHOULD BE MADE THROUGH NIMA AT THE ADDRESS ABOVE.
     * </p>
     * <p>
     *
     *
     *
     * NOTE: THIS VERSION OF GEOMAG USES THE WMM-2010 GEOMAGNETIC MODEL
     * REFERENCED TO THE WGS-84 GRAVITY MODEL ELLIPSOID
     * </p>
     *
    ...
    private void calcGeoMag(double fLat, double fLon, double year, double altitude)
    {
        ...
        alt = altitude;
        /**
         * The date in decimal years for calculating the magnetic field
         * components.
         */
        time = year;
        ...
        /* gv = -999.0; if (Math.abs(glat) >= 55.){ if (glat > 0.0 && glon >=
         * 0.0) gv = dec-glon; if (glat > 0.0 && glon < 0.0) gv = dec +
         * Math.abs(glon); if (glat < 0.0 && glon >= 0.0) gv = dec+glon; if
         * (glat < 0.0 && glon < 0.0) gv = dec - Math.abs(glon); if (gv >
         * +180.0) gv -= 360.0; if (gv < -180.0) gv += 360.0; } */
        otime = time;
        oalt = alt;
        olat = glat;
        olon = glon;
    ...
    /**
     * Returns the declination from the Department of Defense geomagnetic model
     * and data, in degrees. The magnetic heading + declination = true heading.
     * The date and altitude are the defaults, of half way through the valid 5
     * year period, and 0 elevation. (True heading + variation = magnetic
     * heading.)
     *
     ...
     * geomagnetic model and data in nano Tesla. The date and altitude are the
    ...
    /**
     * Returns the magnetic field intensity from the Department of Defense
     * geomagnetic model and data in nano Tesla.
     *
     * @param dlat Latitude in decimal degrees.
     * @param dlong Longitude in decimal degrees.
     ...
     * Defense geomagnetic model and data in nano Tesla. The date and altitude
     * are the defaults, of half way through the valid 5 year period, and 0
     * elevation.
     *
     * @param dlat Latitude in decimal degrees.
     * @param dlong Longitude in decimal degrees.
    ...
    /**
     * Returns the horizontal magnetic field intensity from the Department of
     * Defense geomagnetic model and data in nano Tesla.
     *
     * @param dlat Latitude in decimal degrees.
     * @param dlong Longitude in decimal degrees.
     * @param year Date of the calculation in decimal years.
    ...
    /**
     * Returns the vertical magnetic field intensity from the Department of
     * Defense geomagnetic model and data in nano Tesla. The date and altitude
     * are the defaults, of half way through the valid 5 year period, and 0
     * elevation.
     *
    ...
    /**
     * Returns the northerly magnetic field intensity from the Department of
     * Defense geomagnetic model and data in nano Tesla. The date and altitude
     * are the defaults, of half way through the valid 5 year period, and 0
     * elevation.
     *
     ...
     * @param dlong Longitude in decimal degrees.
     *
     * @return The northerly component of the magnetic field strength in nano
     *         Tesla.
     */
    ...
    /**
     * Returns the easterly magnetic field intensity from the Department of
     * Defense geomagnetic model and data in nano Tesla. The date and altitude
     * are the defaults, of half way through the valid 5 year period, and 0
     * elevation.
     *
     ...
     * @param dlong Longitude in decimal degrees.
     *
     * @return The easterly component of the magnetic field strength in nano
     *         Tesla.
     */
    ...
    /**
     * This method sets the input data to the internal fit coefficents. If there
     * is an exception reading the input file WMM.COF, these values are used.
     *
     * NOTE: This method is not tested by the JUnit test, unless the WMM.COF
     * file is missing.
     */
    ...
    /**
     * <p>
     * Given a Gregorian Calendar object, this returns the decimal year value
     * for the calendar, accurate to the day of the input calendar. The hours,
     * minutes, and seconds of the date are ignored.
     * </p>
     * <p>
     *
     * If the input Gregorian Calendar is new GregorianCalendar(2012, 6, 1), all
     * of the first of July is counted, and this returns 2012.5. (183 days out
     * of 366)
     * </p>
     * <p>
     *
