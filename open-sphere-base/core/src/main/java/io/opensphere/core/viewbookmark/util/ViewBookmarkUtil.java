...
{
    /**
     * Center the viewer on the position within the view.
     *
     * @param toolbox the {@link Toolbox}
     * @param view the {@link ViewBookmark} to go to
     */
    public static void centerView(final Toolbox toolbox, final ViewBookmark view)
    {
        ...
        if (view != null)
        {
            ...
            if (privateViewer instanceof Viewer3D)
            {
                if (view.is3D())
                {
                    ...
                    new ViewerAnimator(view3d, view.getViewerPos3D(), false).start();
                }
                else
                {
                            ...
                            "Projection Change Required", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION)
                    {
                        ...
                        EventQueueUtilities.runOnEDT(() ->
                        {
                            ...
                            if (viewer instanceof Viewer2D)
                            {
                                ...
                                new ViewerAnimator(view2D, view.getViewerPos2D(), true).start();
                            }
                        });
                    }
                }
            ...
            else if (privateViewer instanceof Viewer2D)
            {
                if (view.is2D())
                {
                    ...
                    new ViewerAnimator(view2d, view.getViewerPos2D(), false).start();
                }
                else
                {
                    if (JOptionPane.showConfirmDialog(toolbox.getUIRegistry().getMainFrameProvider().get(),
                            ...
                            "Projection Change Required", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION)
                    {
                        ...
                        EventQueueUtilities.runOnEDT(() ->
                        {
                            ...
                            if (viewer instanceof Viewer3D)
                            {
                                ...
                                new ViewerAnimator(view3D, view.getViewerPos3D(), true).start();
                            }
                        });
                    }
                }
    ...
    /**
     * Reposition the viewer on the position within the view, without using
     * confirmation dialogs if the projection view needs to change.
     *
     * @param toolbox the {@link Toolbox}
     * @param view the {@link ViewBookmark} to go to.
     */
    public static void gotoViewWithoutConfirmation(final Toolbox toolbox, final ViewBookmark view)
    {
        Utilities.checkNull(toolbox, "toolbox");
        if (view != null)
        {
