...
/**
 * This is a proof-of-concept class for performing HTTP route planning in the new HTTP Client API.
 * <p>
 * <b>WARNING</b>: This class will likely change in the future or go away so use it at your own
 * risk!
 */
public class DynamicProxyRoutePlanner implements HttpRoutePlanner {
   ...
   public HttpRoute determineRoute(final HttpHost target, final HttpRequest request,
      ...
      try {
         ...
         throw new HttpException("Failed to determine the proxy server for target=" + target
            + ", request=" + request + ", context=" + context, e);
      }
      ...
      if (list.isEmpty()) {
         ...
         case SOCKS:
            throw new HttpException(
               "The first recommended proxy configuration is SOCKS which is not supported: "
                  + config);
         case DIRECT:
         default:
            httpRoute = new HttpRoute(target, null, isSecure);
