...
public final class KeyStoreUtils
{
    ...
    *            if unable to create a key store using the value from
    ...
    */
   public static KeyStore getSystemKeyStore() throws KeyStoreException,
      NoSuchAlgorithmException, CertificateException, IOException
   {
      return loadKeyStore(getSystemKeyStorePath(),
    ...
    * @throws NoSuchAlgorithmException
    *            if the algorithm used to check the integrity of the trust store
    *            cannot be found.
    * @throws CertificateException
    *            if any of the certificates in the trust store could not be
    *            loaded.
    * @throws IOException
   ...
   public static KeyStore getSystemTrustStore() throws KeyStoreException,
      NoSuchAlgorithmException, CertificateException, IOException
   {
      return loadKeyStore(getSystemTrustStorePath(),
                          System.getProperty(TRUST_STORE_PASSWORD, ""),
                          System.getProperty(TRUST_STORE_TYPE,
                                             KeyStore.getDefaultType()));
   }
    ...
    * @param typeHint
    *           the hint for the key store type or <code>null</code>.
   ...
   private static KeyStore loadKeyStore(final String keyStorePath,
                                        final String password, final String typeHint)
    ...
    *            <code>ArithmeticException</code>.
    */
   public static KeyStore loadKeyStore(final String keyStorePath,
                                       final char[] password, final String typeHint)
      throws KeyStoreException, NoSuchAlgorithmException, CertificateException,
      IOException
   {
    ...
    *            <code>ArithmeticException</code>.
    */
   public static KeyStore loadKeyStore(final URL keyStoreUrl, final char[] password,
      ...
      IOException
   {
      ...
      for (final String type : getSupportedKeyStoreTypes(typeHint))
      {
         ...
         {
            // If the IOException's cause is an UnrecoverableKeyException or an
            // ArithmeticException, then the password is incorrect so just throw
            // the exception.
   ...
   /**
    * Returns the set of supported key store types using the given type hint. If
    * the hint is specified, it will be the first type in the set.
    *
    * @param typeHint
    *           an optional key store type.
