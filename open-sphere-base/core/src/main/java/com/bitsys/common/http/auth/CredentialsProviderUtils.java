...
public final class CredentialsProviderUtils
{
   ...
   {
      return new AuthScope(scope.getHost(), scope.getPort(), scope.getRealm(),
         scope.getScheme());
   }
    ...
    *
    * @param authScope
    *           the Apache authentication scope.
    * @return the converted authentication scope or <code>null</code> if the
    *         given scope is <code>null</code>.
    */
   ...
   public static Credentials toCredentials(final org.apache.http.auth.Credentials credentials)
   {
      ...
      {
         final org.apache.http.auth.UsernamePasswordCredentials upCredentials =
            (org.apache.http.auth.UsernamePasswordCredentials)credentials;
         creds =
            new UsernamePasswordCredentials(upCredentials.getUserName(),
               upCredentials.getPassword().toCharArray());
      }
   ...
   public static org.apache.http.auth.Credentials toCredentials(final Credentials credentials)
   {
      ...
      {
         final UsernamePasswordCredentials upCredentials =
            (UsernamePasswordCredentials)credentials;
         final char[] password = upCredentials.getPassword();
         creds =
            new org.apache.http.auth.UsernamePasswordCredentials(
               upCredentials.getUserName(), new String(password));
