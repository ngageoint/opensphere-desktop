...
public class ProxyScriptRunner {
    ...
    *            if unable to initialize the scripting engine.
    * @throws UnknownHostException
    *            if unable to determine the current host address.
    */
   ...
   /**
    * Loads deploy.jar and fetches the system scripts from <code>AutoProxyScript</code>.
    */
   protected synchronized static void initialize() {
      if (!deployJarLoaded.get()) {
         deployJarLoaded.set(true);
         try {
            // Attempt to retrieve the system scripts first in case deploy.jar is already in the
            // classpath.
            ...
            if (addDeployJar()) {
               try {
                  ...
                  throw new IllegalStateException(
                     "Ensure that JAVA_HOME/lib/deploy.jar is in your classpath", e);
               }
            }
         }
      }
   ...
   protected synchronized static boolean addDeployJar() {
      ...
      if (cl instanceof URLClassLoader) {
         ...
         final String deployJarPath =
            javaHome + File.separator + "lib" + File.separator + "deploy.jar";
         ...
         if (deployJar.isFile() && deployJar.canRead()) {
            ...
            LOGGER.warn(deployJarPath
               + " is not a file or is not readable.  Please ensure you are using Oracle Java.");
         }
      }
      else {
    ...
    *            if unable to load <code>AutoProxyScript</code>.
    */
   protected synchronized static void retrieveSystemScripts() throws ClassNotFoundException {
      ...
      for (final Field field : fields) {
         try {
            ...
            throw new IllegalStateException("Failed to retrieve the value for AutoProxyScript."
               + field.getName(), e);
         }
    ...
    *           the script to add.
    * @throws ScriptException
    *            if an error occurs while processing the new script.
    */
   ...
   /**
    * Invokes the <code>FindProxyForURL</code> proxy function to determine the appropriate proxy
    * server configuration for the given URL/host combination.
    *
    * @param url
    *           the destination URL.
    * @param hostName
    *           the destination host name.
    * @return the list of <code>ProxyHostConfig</code>s. They should be attempted in order until one
    *         succeeds.
    * @throws ScriptException
    *            if an error occurs while invoking <code>FindProxyForURL</code>.
    * @throws NoSuchMethodException
    *            if the <code>FindProxyForURL</code> method does not exist. Pass the script to
    *            {@link #addScript(Reader)} that defines <code>FindProxyForURL</code>.
    */
   public List<ProxyHostConfig> findProxyForUrl(final URL url, final String hostName)
      ...
      final Object retValue =
         ((Invocable)scriptEngine)
            .invokeFunction("FindProxyForURL", url.toExternalForm(), hostName);
      ...
      if (retValue != null) {
         ...
         for (int index = 0; index < results.length; index++) {
            ...
            if (result.equalsIgnoreCase("null") || StringUtils.isEmpty(result)) {
               LOGGER.debug("Ignoring a 'null' or empty result for configuration " + index + ": "
                  + results[index]);
               continue;
            }
            ...
            try {
               ...
               LOGGER.debug("Unexpected auto-proxy type for configuration " + index + ": "
                  + parts[0], e);
               continue;
            }
            ...
            if (proxyType == null) {
               LOGGER.debug("Ignoring unknown auto-proxy type for configuration " + index + ": "
                  + parts[0]);
               continue;
            }
            ...
            if (proxyType == ProxyType.DIRECT && parts.length > 1) {
               LOGGER.debug("DIRECT proxy type contains too much information for configuration "
                  + index + ": " + result);
            }
               ...
               if (parts.length > 2) {
                  try {
                     ...
                     LOGGER.warn("Failed to parse the port for configuration " + index + ": "
                        + parts[2]);
                  }
                  if (parts.length > 3) {
                     LOGGER.debug("Unexpected information after port found for configuration "
                        + index + ": " + result);
                  }
               }
            }
