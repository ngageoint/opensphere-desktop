//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.8-b130911.1802 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2017.02.22 at 10:22:41 AM MST 
//

package com.bitsys.fade.mist.state.v4;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;

/**
 * <p>
 * Java class for ProjectionType.
 * 
 * <p>
 * The following schema fragment specifies the expected content contained within
 * this class.
 * <p>
 * 
 * <pre>
 * &lt;simpleType name="ProjectionType">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}token">
 *     &lt;enumeration value="Perspective"/>
 *     &lt;enumeration value="Equirectangular"/>
 *     &lt;enumeration value="Mercator"/>
 *     &lt;enumeration value="Miller"/>
 *     &lt;enumeration value="Hammer"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "ProjectionType")
@XmlEnum
public enum ProjectionType
{

    @XmlEnumValue("Perspective")
    PERSPECTIVE("Perspective"), @XmlEnumValue("Equirectangular")
    EQUIRECTANGULAR("Equirectangular"), @XmlEnumValue("Mercator")
    MERCATOR("Mercator"), @XmlEnumValue("Miller")
    MILLER("Miller"), @XmlEnumValue("Hammer")
    HAMMER("Hammer");
    private final String value;

    ProjectionType(String v)
    {
        value = v;
    }

    public String value()
    {
        return value;
    }

    public static ProjectionType fromValue(String v)
    {
        for (ProjectionType c : ProjectionType.values())
        {
            if (c.value.equals(v))
            {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
